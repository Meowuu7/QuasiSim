...
def parse_data_from_urdf(xml_fn):
    ...
    for i_joint, cur_joint in enumerate(cur_joints):
        ...
        print(f"cur_joint_name: {cur_joint_name}, cur_joint_idx: {cur_joint_idx}")
...
class RobotAgent: # robot and the robot #
    ...
    def set_init_states_target_value(self, init_states):
        ...
        # glb_trans = torch.zeros((3,), dtype=torch.float32).cuda() ### glb_trans #### and the rot ##
...
def calibreate_urdf_files(urdf_fn):
    ...
    for cur_joint in tot_joints:
        ...
        scaled_joint_origin = cur_joint_origin * scale_factor
    ...
    changed_urdf_fn = urdf_fn.replace(".urdf", f"_scaled_{scale_factor_str}.urdf")
