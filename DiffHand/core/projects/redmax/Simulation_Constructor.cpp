...
std::vector<Vector3> Simulation::parse_contact_points(std::string str) { // contact points? 
...
Joint* Simulation::parse_from_xml_file(pugi::xml_node root, pugi::xml_node node,
    ...
    if ((std::string)(node.name()) == "redmax") { // 
            ...
            if ((std::string)(contact.name()) == "contact") {
                for (auto child : contact.children()) {
                    ...
                    if ((std::string)(child.name()) == "ground_contact") {
                        ...
                        robot->add_force(force); // primitive //
                    } else if ((std::string)(child.name()) == "virtual_force") {
                        auto it1 = _body_map.find(child.attribute("general_body").value());
                        if (it1 == _body_map.end()) {
                            std::string error_msg = "General contact body name error: " + (std::string)(child.attribute("general_body").value());
                            throw_error(error_msg);
                        ...
                        ForceGeneralVirtual* force = new ForceGeneralVirtual(this, it1->second, it2->second, kn, kt, mu, damping, render_contact_points);
                        robot->add_force(force); // primitive //
                    }
                }
            }
            ...
            if ((std::string)(variable.name()) == "variable") { // 
                for (auto child : variable.children()) {
                    if ((std::string)(child.name()) == "endeffector") {
                        ...
                        if (child.attribute("rgba")) { // rgba // end effector //
                ...
                for (auto child : virtuals.children()) {
                    ...
                    if ((std::string)(child.name()) == "sphere") {
                        ...
                        if (child.attribute("rgba")) {
                            rgba = str_to_eigen(child.attribute("rgba").value()); // virtual object sphere //
                        }
        ...
        if (node.attribute("design_params")) { // read the design aprams from the xml file and 
        ...
        if (node.child("joint")) {
            ...
            if (design_params_1) {
                joint->activate_design_parameters_type_1(true); // hand deformation based design parameter other than the free-form //
            }

            if (design_params_5) { // hand and deformation based design parameter other thna the free-form // free 
                joint->activate_design_parameters_type_5(true); // hand 
            }
        }
