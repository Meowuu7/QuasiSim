...
class ForceGeneralVirtual;

class ForceGeneralVirtualAnimator : public opengl_viewer::Animator {
public:
    ForceGeneralVirtualAnimator(ForceGeneralVirtual* force) : _force(force) {}

    const Eigen::Matrix4f AnimatedModelMatrix(const float t);

    ForceGeneralVirtual* _force;
};

class ForceGeneralVirtual : public Force {
public:
    ...
    VectorX virual_force;

    std::vector<Contact> _contacts;

    ForceGeneralVirtual( // force gneral virutal // force //
        Simulation* sim,
