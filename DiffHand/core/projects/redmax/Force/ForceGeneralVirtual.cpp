...
#include "Force/ForceGeneralVirtual.h"
...
// animated mode 
const Eigen::Matrix4f ForceGeneralVirtualAnimator::AnimatedModelMatrix(const float t) {
...
ForceGeneralVirtual::ForceGeneralVirtual(
    Simulation* sim,
...
void ForceGeneralVirtual::set_stiffness(dtype kn, dtype kt) {
    _kn = kn;
    _kt = kt;
}

void ForceGeneralVirtual::set_friction(dtype mu) {
    _mu = mu;
}

void ForceGeneralVirtual::set_damping(dtype damping) {
    _damping = damping;
}

void ForceGeneralVirtual::set_scale(dtype scale) {
    _scale = scale;
}



void ForceGeneralVirtual::set_forces(VectorX fm) { 
    virual_force = VectorX::Zero(6); // set to zero forces //
    // virual_force = fm;
    // virual_force.segment(0, 6) += fm.head(6); 
    // _contact_body->_index[0]
    virual_force.segment(0, 6) += fm.segment(_contact_body->_index[0], 6); 
}

// kinematics actions and virtual forces added by the contacts? //
// compute forces 

// force general primitive contact //
void ForceGeneralVirtual::computeForce(VectorX& fm, VectorX& fr, bool verbose) {
    auto t0 = clock();
    ...
    // VectorX fm_sub; // how to compute forces from the contacts? // 
    // computeForce(_contacts, fm_sub, verbose); // contact body segments //  // 

    // the contact forces // contact forces are added to the contact body //
    ////// strategy 5 ---- single directional forces ////// // 
    // fm.segment(_contact_body->_index[0], 6) += fm_sub.head(6); // contact body fm sub // // fm_sub and fm_sub and fm_sub ///
    // virual_force // segment of the fm += 
    fm.segment(_contact_body->_index[0], 6) += virual_force.head(6); 
    ////// strategy 5 ---- single directional forces //////
...
void ForceGeneralVirtual::computeForceWithDerivative( // 
    ...
    // detect the contact points between two bodies //
    // _contacts.clear();
    // general primitive collision detection //
    // collision_detection_general_primitive(_contact_body, _primitive_body, _contacts); //
...
// // private
// void ForceGeneralVirtual::computeForce(std::vector<Contact> &contacts, VectorX& fm, bool verbose) {
//     Matrix3 R1 = _contact_body->_E_0i.topLeftCorner(3, 3); // R1 // // R 
...
//     fm = VectorX::Zero(12);
//     Km = MatrixX::Zero(12, 12); // matrices //
//     Dm = MatrixX::Zero(12, 12); // matrices //

//     Matrix3 R1 = _contact_body->_E_0i.topLeftCorner(3, 3); // top left corner //
//     Vector6 phi1 = _contact_body->_phi; // top right corner //
//     Vector3 p1 = _contact_body->_E_0i.topRightCorner(3, 1);
//     Matrix3 R2 = _primitive_body->_E_0i.topLeftCorner(3, 3); // 
//     Vector6 phi2 = _primitive_body->_phi;

//     dtype kn = _kn * _scale * _contact_body->_contact_scale;
//     dtype kt = _kt * _scale * _contact_body->_contact_scale; // contact scale 
//     for (int i = 0;i < contacts.size();i++) { // compute force with derivative 
//         Vector3 xi1 = contacts[i]._xi;
//         Vector3 xw1 = R1 * xi1 + p1;  // spatial position //
//         Vector3 xw1_dot = R1 * (math::skew(xi1).transpose() * phi1.head(3) + phi1.tail(3));  // spatial position //
        
//         Matrix36 dxw1_dq1; 
//         dxw1_dq1.leftCols(3) = -R1 * math::skew(xi1); // q1 -> xi1? //
...
//         dxw1dot_dphi1.leftCols(3) = R1 * math::skew(xi1).transpose(); //dxw1dot_dphi1 //
...
//         Matrix36 dn_dq2, dtdot_dq2, dtdot_dphi2, dxi2_dq2; // xi2 // tdot //
//         _primitive_body->collision(xw1, xw1_dot, d, n, ddot, tdot, xi2, // xi2 // tdot // tdot // // tdot // 
//                                     dd_dxw1, dd_dq2, // 
//                                     dn_dxw1, dn_dq2, // 
...
//                                     dxi2_dxw1, dxi2_dq2); //dxi2_dq2 //

//         Matrix36 dxi2_dq1 = dxi2_dxw1 * dxw1_dq1; // dxi2 // what is the dxi2?
...
//         JacobianMatrixVector dGamma2_dq2(3, 6, 6); // dGamma
//         dGamma2_dq2.setZero();
//         for (int j = 0;j < 6;j++)
//             for (int k = 0;k < 3;k++)
...
//                 // derivatives
//                 Km.topLeftCorner(6, 6) -= kt * GTRT1 * dtdot_dq1; // topleftcorner
...
//     Matrix3 R1 = _contact_body->_E_0i.topLeftCorner(3, 3); // topleftcorner //
//     Vector6 phi1 = _contact_body->_phi;
...
//     for (int i = 0;i < contacts.size();i++) {
//         Vector3 xi1 = contacts[i]._xi;
//         Vector3 xw1 = _contact_body->_E_0i.topLeftCorner(3, 3) * xi1 + _contact_body->_E_0i.topRightCorner(3, 1); // xw1 -> position?
//         Matrix3 xi1_brac = math::skew(xi1);
//         Vector3 xi1_dot = xi1_brac.transpose() * phi1.head(3) + phi1.tail(3); // 
//         Vector3 xw1_dot = R1 * xi1_dot;  // not very sure what does the xw1_dot mean here //
//         int contact_id = contacts[i]._id; // contact idx //

//         Matrix36 dxw1_dq1;
//         dxw1_dq1.leftCols(3) = -R1 * math::skew(xi1); // dxw1_dq1 -> the gradient from the contact point position to the generalized coordinate //
//         dxw1_dq1.rightCols(3) = R1;
...
//         // contact force
//         // contact force on general contact body // general contact bodies //
//         Vector6 fc1 = (-kn * d + _damping * ddot * d) * ni1;
...
void ForceGeneralVirtual::get_rendering_objects(
    ...
    if (_render_contact_points) {
        ...
        _animator = new ForceGeneralVirtualAnimator(this);
...
void ForceGeneralVirtual::test_derivatives() {
...
void ForceGeneralVirtual::test_derivatives_runtime() {
    // detect the contact points between two bodies
    std::vector<Contact> contacts;
    contacts.clear();
...
void ForceGeneralVirtual::test_design_derivatives_runtime() {
    // detect the contact points between two bodies
    std::vector<Contact> contacts;
    contacts.clear();
