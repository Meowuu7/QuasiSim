...
void Robot::add_force(Force* force) {
    _forces.push_back(force); // force and _force //
}
...
// init robot // construct dfs order // compute forces //
...
void Robot::init(bool verbose) {
    ...
    // make array for joints and bodies in bfs/dfs order // order //
    _joints.clear();
    ...
    for (auto body : _bodies) {
        body->_index.clear(); // body index clear //
        for (int i = 0;i < 6;++i)
    ...
    for (auto tactile_sensor : _tactile_sensors) {
        _ndof_tactile += tactile_sensor->_pos_i.size() * 3; // tactile sensor //
    }
    ...
    for (auto body : _bodies) {
        if (body->_design_params_2._active) // the denoising model would totally would totally fail 
    ...
    _ndof_p = _ndof_p1 + _ndof_p2 + _ndof_p3 + _ndof_p4 + _ndof_p5 + _ndof_p6; // and the ndof_p6s for the 
    ...
    _initial_c_force = VectorX::Zero(_ndof_m); 
    // _initial_c_force += 1000;
    for (int i = 0; i < _ndof_m; i++) {
        _initial_c_force[i] = 1000;
    }


    _prev_c_force = VectorX::Zero(_ndof_m);
    _cur_c_force = VectorX::Zero(_ndof_m); 
...
VectorX Robot::get_ctrl_force() {
    ...
    for (auto actuator : _actuators) { // actuators //
...
void Robot::set_forces(VectorX fm) {
    for (auto force : _forces) {
        force->set_forces(fm); // set the virtual forces // //
    }
}
...
void Robot::computeForce(VectorX& fm, VectorX& fr) {
    ...
    fr.setZero(); // compute forces //
    for (auto joint : _joints) 
        joint->computeJointForce(fr); // compute joint force // joint forces //
    // std::cerr << "----------------------------- new iteration -----------------------------" << std::endl;


    // with tmpfm to export the collision forces // set thm via the order??
    VectorX tmp_fm = VectorX::Zero(_ndof_m); // tmp_fm for the vectorX //
    
    for (auto force : _forces)
        // force->computeForce(fm, fr); // // 
        force->computeForce(tmp_fm, fr);
    
    /// strategy 0 --- zerofy the force //// 
    // tmp_fm = VectorX::Zero(_ndof_m);
    /// strategy 0 --- zerofy the force //// 
    
    /// strategy 1 --- add the discounte factors ///
    // float alpha = 0.9;
    // tmp_fm = _initial_c_force * alpha;
    // _initial_c_force = _initial_c_force * alpha;
    // /// strategy 1 --- add the discounte factors ///

    // /// strategy 2 --- interpolating across the previous forcce ///
    // float interp_alpha = 0.9; 
    // tmp_fm = interp_alpha * _prev_c_force + (1. - interp_alpha) * tmp_fm;
    // _cur_c_force = tmp_fm;
    /// strategy 2 --- interpolating across the previous forcce ///
    
    // give it a temporal changing constants? // temp_fm --- many forces here //
    fm += tmp_fm; // add to the fm // // 
    
    // open the file and exprot forces // // forces // 
...
void Robot::computeForceWithDerivative(VectorX& fm, VectorX& fr, MatrixX& Km, MatrixX& Dm, MatrixX& Kr, MatrixX& Dr, bool verbose) {
    ...
    Dm.setZero();
    for (auto body : _bodies) {
        body->computeMaximalForceWithDerivative(fm, Km, Dm); // maximal force with derivative //
    }
    fr.setZero();
    Kr.setZero();
    Dr.setZero();
    for (auto joint : _joints) 
        joint->computeJointForceWithDerivative(fr, Kr, Dr); // joint force with derivatives //


    //// Original strategy ////
    for (auto force : _forces) 
        force->computeForceWithDerivative(fm, fr, Km, Dm, Kr, Dr, verbose); // force with derivatives // force with derivative //
    //// Original strategy ////
    
    //// strateiges where only the forces are changed ////
    // VectorX tmp_fm = VectorX::Zero(_ndof_m);
    // for (auto force : _forces) 
    //     force->computeForceWithDerivative(tmp_fm, fr, Km, Dm, Kr, Dr, verbose); // force with derivatives // force with derivative //
    // // /// strateyg 1 --- zerofy ////
    // // tmp_fm = VectorX::Zero(_ndof_m);
    // // /// strateyg 1 --- zerofy ////
    // // fm += tmp_fm; // 
    ...
    // VectorX tmp_fm = VectorX::Zero(_ndof_m);
    // VectorX tmp_fr = VectorX::Zero(_ndof_r);
    // MatrixX tmp_Km = MatrixX::Zero(_ndof_m, _ndof_m);
    // MatrixX tmp_Dm = MatrixX::Zero(_ndof_m, _ndof_m);
    // MatrixX tmp_Kr = MatrixX::Zero(_ndof_r, _ndof_r);
    // MatrixX tmp_Dr = MatrixX::Zero(_ndof_r, _ndof_r);
    // for (auto force : _forces) 
    //     force->computeForceWithDerivative(tmp_fm, tmp_fr, tmp_Km, tmp_Dm, tmp_Kr, tmp_Dr, verbose); // force with derivatives // force with derivative //
    


    for (auto actuator : _actuators)
        actuator->computeForceWithDerivative(fm, fr, Km, Dm, Kr, Dr); // force with derivative //
}
...
void Robot::computeForceWithDerivative(
    ...
    // VectorX tmp_fm = VectorX::Zero(_ndof_m);
    // for (auto force : _forces) {
    //     force->computeForceWithDerivative(tmp_fm, fr, Km, Dm, Kr, Dr, dfm_dp, dfr_dp, verbose);
    // }
    // /// strateyg 1 --- zerofy ////
    // tmp_fm = VectorX::Zero(_ndof_m);
    // /// strateyg 1 --- zerofy ////
...
void Robot::computeForceWithDerivative( // forces with derivatives //
    VectorX& fm, VectorX& fr, 
    MatrixX& Km, MatrixX& Dm, MatrixX& Kr, MatrixX& Dr, 
    ...
    for (auto force : _forces) { // the forces and the forces //
        force->computeForceWithDerivative(fm, fr, Km, Dm, Kr, Dr, dfm_dp, dfr_dp, verbose); // verbose //
    }
...
void Robot::computeJointJacobian(MatrixX& J, MatrixX& Jdot) {
    ...
    for (auto joint : _joints) { // _joints //
        if (joint->_ndof > 0) {
    ...
    for (auto joint : _joints) {
        ...
        for (Joint* now = parent; now != nullptr; now = now->_parent) {
            if (now->_ndof > 0) {
                ...
                //     joint->_body->_A_ij * tmp1 * now->_S_j
                //     + joint->_body->_A_ij * tmp * now->_S_j_dot;
                // joint -> body->A_ij * tmp * now->_S_j_dot; // 
                for (int k = 0;k < joint->_ndof;k++) {
...
void Robot::computeJointJacobianWithDerivative( // compute jacobian matrix with derivative //
                ...
                if (joint->_body->_design_params_2._active) {
                    ...
                    for (int k = 0;k < joint->_body->_design_params_2._ndof;k++) {
                        ...
                        dJdot_dp2(idx).block(joint->_body->_index[0], now->_index[0], 6, now->_ndof).noalias() += 
                            ...
                            + dAip_dp2 * Jdot.block(Bp->_index[0], now->_index[0], 6, now->_ndof); // Jdot // 
                    }
                }
            }
        }
...
void Robot::computeJointJacobianWithDerivative(
    ...
    for (auto joint : _joints) {   
        ...
        for (Joint* now = Jp; now != nullptr; now = now->_parent) {
            if (now->_ndof > 0) {
                ...
                // design params 2 // active and alpha // 
                if (joint->_body->_design_params_2._active) {
                    // For type II of Bi: 
                    // dJ_dp2(Bi, j) = dAip_dp2 * J(Bp, j)
                    // dJdot_dp2(Bi, j) = dAipdot_dp2 * J(BP, j) + dAip_dp2 * Jdot(Bp, j)
    ...
    for (int k = 0;k < 12;k++) {    // special construction for p2, since p2(Bi) only affects J(Ji, Bi) // J(Ji. Bi) // J(Ji, Bi) // 
        for (auto body : _bodies) {
            if (body->_design_params_2._active) { // 
...
void Robot::test_design_derivatives_runtime() {
    ...
    if (_ndof_p > 0) {
        ...
        // the shape of the primitive shapes; shape of the active object and states -> to the images //
        // shape of the passive object -> approximate it via primitives //
        // gradients to the primitives -> gradients to the passive object with arbitrary geometry // with arbitrary geometry // with arbitrary geometries //
        // obj geometries and the passive geometries //
        // 
        print_error("Robot: dfr_dp", dfr_dp, dfr_dp_fd); // model the passive parameters by actions // model the geometry by actions // model the geometry by actions // 
