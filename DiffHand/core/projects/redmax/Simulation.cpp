...
void Simulation::init(bool verbose) {
    ...
    _fm = VectorX::Zero(_ndof_m); // ndof of the maximal coordinates // VectorX tmp_fm = VectorX::Zero(_ndof_m); 
    ...
    _dM_dq = JacobianMatrixVector(_ndof_r, _ndof_r, _ndof_r); // jacobian matrix vector 
...
void Simulation::update_robot(bool design_gradient) {
    _robot->update(design_gradient); // design gradient !
}
...
void Simulation::computeMatrices(MatrixX& M, VectorX& fr) { // VectorX fr //
    auto t_compute_matrices_start = clock();

    _robot->update(); // compute matrices // // update //
    ...
    VectorX tmp_fm = VectorX::Zero(_ndof_m);  // for computing the contact forces //
    // pass the simulator's fm and fr to the robot for calculation //
    _robot->computeForce(_fm, _fr); // compute forces // contact force changes //
    // reduced forces //
    ...
    auto t_compose_matrices_start = clock(); // clock and the clock //
...
// 
void Simulation::set_forces(VectorX fm) {
    _robot->set_forces(fm); // _robot -> set_forces // 
}
...
void Simulation::computeMatrices(MatrixX& M, VectorX& fr, JacobianMatrixVector& dM_dq, MatrixX& K, MatrixX& D) {
    ...
    _robot->computeJointJacobianWithDerivative(_J, _Jdot, _dJ_dq, _dJdot_dq); // compute 
    
    _robot->computeMaximalMassMatrix(_Mm);

    auto t_compute_force_start = clock();

    // robot compute force with derivatives //
...
void Simulation::computeMatrices(MatrixX& M, VectorX& fr, JacobianMatrixVector& dM_dq, MatrixX& K, MatrixX& D, MatrixX& dfr_du) {
    ...
    // maximal coordiantes and the reduced coordinate //
    _robot->computeJointJacobianWithDerivative(_J, _Jdot, _dJ_dq, _dJdot_dq);

    // computemaximalmassmatrix //
    _robot->computeMaximalMassMatrix(_Mm);
...
void Simulation::computeMatrices(
    ...
    MatrixX dfm_dp;
    _robot->computeForceWithDerivative(_fm, _fr, _Km, _Dm, _Kr, _Dr, dfm_dp, dfr_dp); /// fm, fr, Km, Dm, Kr, Dr // 
    ...
    // dphi_dq = outer_product(dJ_dq, qdot) # that models the
    // dphi_dq(k) = dJdq(k) * qdot
    for (int k = 0;k < _ndof_r;k++) {
        _dphi_dq.col(k) = _dJ_dq(k) * qdot;
    }
...
void Simulation::computeExtraDerivative(MatrixX& dfr_dqprev, MatrixX& dfr_dqdotprev) {
    _robot->computeExtraDerivative(dfr_dqprev, dfr_dqdotprev); // extra 
}
...
void Simulation::computeMatricesProduct(const VectorX& x, const dtype y, VectorX& g) { 
    ...
    // Jx = J * x
    VectorX Jx;
    _robot->computeJointJacobianProduct(x, Jx); /// joint jacobian 
...
void Simulation::computeMatricesProduct(const VectorX& x, const dtype y, VectorX& g, 
    ...
    VectorX qdot = get_qdot(); // computematricesproduct ****/
...
void Simulation::test_design_derivatives_runtime() {
    ...
    MatrixX K, D;
    MatrixX dfr_du;
    computeMatrices(M, fr, dM_dq, K, D, dfr_du, _dM_dp, _dfr_dp); // matrices // design derivatives // 
...
void Simulation::forward(int num_steps, bool verbose, bool test_derivatives, bool save_last_frame_var_only) {
    ...
    for (int i = 0;i < num_steps;i++) {
        ...
        /// strategy 3 --- interpolating across the previous force ///
        _robot->_prev_c_force = _robot->_cur_c_force;
        /// strategy 3 --- interpolating across the previous force ///
        ...
        _robot->update_actuator_states(q_next, qdot_next); // actuators and actuators states --> how do them affect the states //
...
void Simulation::newton(VectorX& x,
    ...
    bool success_newton = false; // newton //
    dtype g_last;
    int fail_strike = 0;
...
void Simulation::evaluate_g_BDF1(const VectorX& q1, VectorX& g) {
    ...
    g = _M * (q1 - _q0 - _h * _qdot0) - _h * _h * _fr; // qdot; fr // // fr //
}
...
void Simulation::integration_BDF1(
    ...
    qdot1 = (q1 - q0) / h; // (q1 - q0) / h -> get the qdot1 //
...
void Simulation::evaluate_g_BDF2(const VectorX& q2, VectorX& g) { // g BDF2 with derivatives //
...
void Simulation::backward_BDF2() {
    ...
    dtype h = _options->_h;
    for (int k = T;k >= 1;k--) {
        // std::printf("innn.. k: %d; _ndof_r: %d; \n", k, _ndof_r);
        // std::cout << "_dvar_dq_size: " << _backward_info._dvar_dq.size() << std::endl;
        ...
        // std::printf("innn.. k: %d; after setting yk\n", k);
        ...
        // std::printf("innn.. k: %d; after setting yk; start calculating z\n", k);
        z.segment(k * _ndof_r, _ndof_r) = _backward_info._H_lu[k].transpose().solve(yk);
        // std::printf("innn.. k: %d; after setting yk; ending calculating z\n", k);
    }
...
void Simulation::replay() {
    ...
    VectorX q = get_q();
    VectorX qdot = get_qdot(); 
    VectorX virtual_object_data = _robot->get_virtual_object_data(); // virtual 
    init_viewer();
    _viewer->run();
...
void Simulation::export_replay(std::string folder) {
    ...
    std::vector<opengl_viewer::Option> option_list;
    std::vector<opengl_viewer::Animator*> animator_list;
    this->get_rendering_objects(vertex_list, face_list, option_list, animator_list);
    ...
    for (auto body : _robot->_bodies) {
        ...
        // body->_rendering_vertices = body->_V.cast<float>();
        // body->_rendering_faces = body->_F;
        opengl_viewer::WriteToObjFile(body->_rendering_vertices, body->_rendering_faces, path);

        std::printf("idx: %d, %d %d\n", idx, body->_rendering_vertices.rows(), body->_rendering_vertices.cols());
        idx ++;
    }
