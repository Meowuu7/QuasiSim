...
if __name__ == '__main__':
    ...
    target_qs_fn = "/home/xueyi/diffsim/NeuS/optimized_states.npy"
    target_qs = np.load(target_qs_fn, allow_pickle=True)
    print(f"target_qs: {target_qs.shape}")
    ...
    def loss_and_grad_qs(u):
        ...
        tot_qs_np = np.stack(tot_qs, axis=0)
        f = np.sum((tot_qs_np[:target_qs.shape[0], :ndof_u] - target_qs) ** 2, axis=-1).mean()
        ...
        # reconstructor.forward_p2g() # goal optimize -> then with the dense sup optimize ### sup optimize ### 
        # reconstructor.forward_ti_rendering()
        
        # reconstructor.initialize()
        
        # df_du #
        print(f"iter: {i_iter}, q: {tot_qs_np[:target_qs.shape[0]]}")
        sim.backward_info.set_flags(False, False, False, True) 
        ...
        for i_s in range(target_qs.shape[0]):
            cur_st_dof = ((i_s + 1) * nn_mod_steps) * ndof_r 
            cur_ed_dof = ((i_s + 1) * nn_mod_steps) * ndof_r + ndof_u
            df_dq[cur_st_dof: cur_ed_dof] = 2 * (tot_qs_np[i_s, :ndof_u] - target_qs[i_s])
        # df_dq[-ndof_r: -(ndof_r - ndof_u)] = 2 * (q - init_q_goal)[: -(ndof_r - ndof_u)]
        ### dense states supervision ####
        # tot_qs = np.concatenate(tot_qs, axis=0)
        # df_dq = 2 * tot_qs ### tot_q
    ...
    tot_frame_act_xs = None
    
    
    def callback_func_qs(u, render=False):
        global tot_frame_act_xs, tot_frame_passive_xs
        sim.reset(False)
        ...
        tot_frame_act_xs, tot_frame_passive_xs = reconstructor.forward_stepping()
    ...
    # "/home/xueyi/diffsim/NeuS/optimized_states.npy"
    optimized_us_sv_fn = "/home/xueyi/diffsim/NeuS/optimized_us.npy"
    # tot_frame_act_xs
    optimized_infos = {
        'tot_frame_act_xs': tot_frame_act_xs.detach().cpu().numpy(),
        'tot_frame_passive_xs': tot_frame_passive_xs.detach().cpu().numpy(),
        'u': u
    }
    np.save(optimized_us_sv_fn, u)
    
    optimized_infos_sv_fn = "/home/xueyi/diffsim/NeuS/optimized_infos.npy"
    np.save(optimized_infos_sv_fn, optimized_infos)
    exit(0)
