...
class SimRenderer:
    ...
    def replay(sim, record = False, record_path = None):
        ...
        # print(f"record path: {record_folder}")
        sim.replay()
        # print(f"record path: {record_folder}")
        if record:
            images_path = os.path.join(record_folder, r"%d.png")
            os.system("/Users/xymeow/Downloads/ffmpeg -i {} -vf palettegen palette.png".format(images_path))
            os.system("/Users/xymeow/Downloads/ffmpeg -i {} -i palette.png -lavfi paletteuse {}".format(images_path, record_path))
