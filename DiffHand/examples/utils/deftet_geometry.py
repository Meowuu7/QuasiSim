...
def marching_tets_tetmesh(
    ...
    with torch.no_grad():
        occ_n = sdf_n > 0
        occ_fx4 = occ_n[tet_fx4.reshape(-1)].reshape(-1, 4) ### occ_fx4: occ_fx4 for 
        occ_sum = torch.sum(occ_fx4, -1)
    ...
    edges_to_interp_sdf = torch.flip(edges_to_interp_sdf, [1]) / denominator ### sdf for edges
    verts = (edges_to_interp * edges_to_interp_sdf).sum(1)
...
#  Geometry interface
###############################################################################
class DMTetGeometry(Geometry): # geomtry
    def __init__( 
        ...
        # if grid_res > 100:
        if grid_res > 100:
            tets = np.load('/home/xueyi/sim/MeshDiffusion/nvdiffrec/data/tets/%d_tets_cropped.npz' % (grid_res))
            ...
            maxx_verts, _ = torch.max(self.verts, dim=0)
            minn_verts, _ = torch.min(self.verts, dim=0)
            print(f"maxx_verts: {maxx_verts}, minn_verts: {minn_verts}")
            
        else:
            # /home/xueyi/gen/GET3D/data
            tets = np.load('/home/xueyi/gen/GET3D/data/tets/%d_compress.npz' % (grid_res))
            ## verts 
            self.verts = torch.from_numpy(tets['vertices']).float().to(self.device)
        ...
        ## create them for creating the dmtet optimizable variables ##
        print(f"verts: {self.verts.size()}, indices: {self.indices.size()}")
        
        # self.verts #
        self.sdf_verts = nn.Parameter(torch.zeros((self.verts.size(0),), dtype=torch.float32, requires_grad=True).to(self.device), requires_grad=True)
        self.def_verts = nn.Parameter(torch.zeros((self.verts.size(0), 3), dtype=torch.float32, requires_grad=True).to(self.device), requires_grad=True) ## 
    
        # self.sdf_verts.data -= 1.
        self.sdf_verts.data = torch.randn_like(self.sdf_verts.data) * 2.
    
    
    def get_tet_mesh_optimizable(self, ):
        # get the tet mesh from here #
        # get the tet #
        curr_verts = self.def_verts + self.verts ### get the current verts ###
        # to the to the current 
        indices = self.indices # self.indices #
        # 
        verts, faces, tet_verts, tets = marching_tets_tetmesh( ## triangle_mesh ##
            curr_verts, self.sdf_verts, indices, self.triangle_table,
            self.num_triangles_table, self.base_tet_edges, self.v_id, return_tet_mesh=True,
            num_tets_table=self.num_tets_table, tet_table=self.tet_table, ori_v=curr_verts)
        faces = torch.cat(
            [faces[:, 0:1],
             faces[:, 2:3],
             faces[:, 1:2], ], dim=-1)
        print(f"marching tet result ---- verts: {verts.size()}, faces: {faces.size()}")
        return verts, faces, tet_verts, tets
    
    def print_grads(self, ):
        # grads #
        print(f"sdf_verts_grad: {self.sdf_verts.grad}")
        print(f"def_verts_grad: {self.def_verts.grad}") ### def_verts ### ### def_verts ###
        # sdf 
    ...
    ## v_deformed_nx3 # ## deformed_nx3 --> deformed_nx3 
    ...
    def render_mesh(self, mesh_v_nx3, mesh_f_fx3, camera_mv_bx4x4, resolution=256, hierarchical_mask=False):
        ...
        if self.render_type == 'neural_render':
            tex_pos, mask, hard_mask, rast, v_pos_clip, mask_pyramid, depth = self.renderer.render_mesh(
                mesh_v_nx3.unsqueeze(dim=0), ## v_pos_clip ## `## v_pos_clip ##`
                mesh_f_fx3.int(),
                camera_mv_bx4x4,
    ...
    def render(self, v_deformed_bxnx3=None, sdf_bxn=None, camera_mv_bxnviewx4x4=None, resolution=256):
        # Here I assume a batch of meshes (can be different mesh and geometry), for the other shapes, the batch is 1 ## the batch is 1 ##
        v_list = []
        f_list = []
...
if __name__=='__main__':
    ...
    # # DMTetGeometry
    # 
    # CUDA_VISIBLE_DEVICES=1 python utils/deftet_geometry.py
    # verts: torch.Size([36562, 3]), indices: torch.Size([192492, 4]) #
    grid_res = 64
    scale = 2.0
    th_cuda_idx = 1
    device = f'cuda:{th_cuda_idx}'
    render_type = 'neural_render'
    dmtet_geometry = DMTetGeometry(
        grid_res=grid_res, scale=scale, device=device, renderer=None, render_type=render_type, args=None
    )
