...
# passive_robot = RigidBody(filename)
class RigidBody:
    def __init__(self, filename) -> None:
        self.filename = filename
        self.get_visual_counterparts() # get visual counterparts #
        self.nn_pts = self.visual_pts_ref.size(0)
        self.rgba = np.zeros((4,), dtype=np.float32)
        self.rgba = torch.from_numpy(self.rgba).float().cuda()
        self.rigid_translations = torch.tensor(
            [[ 0.0000,  0.0000,  0.0000],
            [-0.0008,  0.0040,  0.0159],
            [-0.0566,  0.0099,  0.0173]], dtype=torch.float32
        ).cuda()
        self.rigid_rotations = torch.zeros_like(self.rigid_translations)
        self.rigid_transformations = torch.cat(
            [self.rigid_rotations, self.rigid_translations], dim=-1 ### nn_frames x 6 ###
        )
        pass
    
    def get_visual_counterparts(self, ):
        body_mesh = trimesh.load(self.filename, process=False) 
        ...
        self.visual_pts_ref = torch.tensor(body_mesh.vertices, dtype=torch.float32, requires_grad=True).cuda()
        # self.visual_pts_ref.re
        self.visual_pts_ref = nn.Parameter(
            self.visual_pts_ref, requires_grad=True
        )
        # self.jv
        ...
        #     self.full_main_axis_pts = full_main_axis_pts
        # else:
        #     self.full_main_axis_pts = mean_pts.clone() ### get the mean pts ###
    
    # def 
    def transform_visual_pts(self, state_vec):
        # if self.name == "sphere":
        #     visual_pts_ref =  self.visual_pts_ref  / 2. #
        #     visual_pts_ref = visual_pts_ref * self.radius
        # else:
        #     visual_pts_ref = self.visual_pts_ref
        visual_pts_ref = self.visual_pts_ref + state_vec[-3:].unsqueeze(0) # visual pts ref #
        return visual_pts_ref
    
    def clear_grad(self):
        if self.visual_pts_ref.grad is not None:
            self.visual_pts_ref.grad = self.visual_pts_ref.grad * 0.
    
    def get_visual_pts_rgba_values(self, visual_pts_rgba_values):
        cur_rgba_values = self.rgba.unsqueeze(0).repeat(self.nn_pts, 1).contiguous()
        visual_pts_rgba_values.append(cur_rgba_values)
        return visual_pts_rgba_values
    
    def compute_transformation_via_state_vecs(self, state_vecs, transformed_pts):
        transformed_visual_pts = self.transform_visual_pts(state_vecs)
        transformed_pts.append(transformed_visual_pts)
...
class Body:
    ...
    def set_radius(self, tar_radius):
        # self.radius.data = tar_radius # #             
        self.radius.data[:] = tar_radius # #                       
...
"""
    ...
    def get_visual_pts_colors(self, ):
        ...
        # self.pts_rgba = [torch.from_numpy(self.rgba).float().cuda() for _ in range(tot_visual_pts_nn)] # total visual pts nn  # totvis pts 
    ...
    def get_visual_counterparts(self,):
        ...
        #### body_mesh.vertices ####
        # verts = torch.tensor(body_mesh.vertices, dtype=torch.float32).cuda()
        # faces = torch.tensor(body_mesh.faces, dtype=torch.long).cuda()
        #### body_mesh.vertices ####
        
        # self.pos = nn.Parameter(
        #     torch.tensor([0., 0., 0.], dtype=torch.float32,  requires_grad=True).cuda(), requires_grad=True
        # )
        ...
        # self.visual_pts_ref = nn.Parameter(
        #     torch.tensor(body_mesh.vertices, dtype=torch.float32, requires_grad=True).cuda(), requires_grad=True
        # )
        # self.visual_faces_ref = nn.Parameter(
        #     torch.tensor(body_mesh.faces, dtype=torch.long, requires_grad=True).cuda(), requires_grad=True
        # )
        self.visual_faces_ref = torch.tensor(body_mesh.faces, dtype=torch.long).cuda()
    ...
    def transform_visual_pts(self, rot_mtx, trans_vec):
        ...
        # full_main_axis_pts -> 
        self.full_main_axis_pts = torch.matmul(rot_mtx, self.full_main_axis_pts.unsqueeze(-1)).contiguous().squeeze(-1) + trans_vec
...
class Link: 
    def __init__(self, name, joint: Joint, body: Body, children, args) -> None:
        ...
        self.tot_trans_vec = nn.Parameter(torch.zeros((3,), dtype=torch.float32).cuda(), requires_grad=True) ## torch zeros #
    ...
    ## 
    def get_tot_transformed_joints(self, transformed_joints):
        cur_joint_transformed_pts = self.joint.transformed_joint_pts ### 3 pts 
...
def dump_robot_configurations(radius, conf_sv_fn):
    target_robot_conf = f"""<redmax model="hand">
<!-- 1) change the damping value here? -->
<!-- 2) change the center of mass -->
    <option integrator="BDF2" timestep="0.01" gravity="0. 0. -0.000098"/>
	<ground pos="0 0 -10" normal="0 0 1"/>
	...
	<robot>
		<link name="sphere">
            ...
            <body name="sphere" type="sphere" radius="{radius}" pos="0 0 0" quat="1 0 0 0" density="0.5" mu="0" texture="resources/textures/sphere.jpg"/>
		</link>
	</robot>
...
"""
    with open(conf_sv_fn, "w") as wf:
        wf.write(target_robot_conf)
...
def load_preoptimized_us(pre_optimized_us_fn):
    pre_optimized_us = np.load(pre_optimized_us_fn, allow_pickle=True)
    print(pre_optimized_us.shape)


if __name__=='__main__':
    
    pre_optimized_us_fn = "/home/xueyi/diffsim/NeuS/optimized_us.npy"
    load_preoptimized_us(pre_optimized_us_fn)
    exit(0)
