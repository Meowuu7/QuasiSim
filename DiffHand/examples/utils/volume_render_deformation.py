...
class VolumeRender:
    ...
    def ray_march_single_loop_with_density_frames_with_deformation(self, field: ti.template(), def_field: ti.template(), angle: ti.f32, view_id: ti.i32, fr: ti.i32): # run the render single loop #
        ...
        for pixel in range(self.res * self.res * self.marching_steps):
            ...
            camera_origin = ti.Vector([ # in the x-z plane #
                self.camera_origin_radius * ti.sin(angle), 0,
                self.camera_origin_radius * ti.cos(angle)
            ])
            dir = ti.Vector([ # dir and the 
                self.fov * (ti.cast(x, ti.f32) /
                    (self.res_f32 / 2.0) - self.res_f32 / self.res_f32),
                self.fov * (ti.cast(y, ti.f32) / (self.res_f32 / 2.0) - 1.0), -1.0
            ])
            ...
            ####### the above information is what the ray would change; -> the ray direction, camera origin #######
    ...
    @ti.kernel
    def ray_march_single_loop_with_density_frames_with_deformation_xy(self, field: ti.template(), def_field: ti.template(), angle: ti.f32, view_id: ti.i32, fr: ti.i32): # run the render single loop #
        ## field; ## iamge; image deformation ##
        for pixel in range(self.res * self.res * self.marching_steps):
            # for k in range(self.marching_steps):
            x = (pixel % (self.res * self.res)) // self.res
            ...
            camera_origin = ti.Vector([ # in the x-y plane #
                self.camera_origin_radius * ti.cos(angle), 
                self.camera_origin_radius * ti.sin(angle), 
                0
            ])
            # dir = ti.Vector([ # dir and the # dir and the ########
            #     self.fov * (ti.cast(x, ti.f32) /
            #         (self.res_f32 / 2.0) - self.res_f32 / self.res_f32),
            #     self.fov * (ti.cast(y, ti.f32) / (self.res_f32 / 2.0) - 1.0), -1.0
            # ])
            ...
            dir = ti.Vector([ # dir and the 
                ...
                self.fov * (ti.cast(x, ti.f32) /
                    (self.res_f32 / 2.0) - self.res_f32 / self.res_f32),
                self.fov * (ti.cast(y, ti.f32) / (self.res_f32 / 2.0) - 1.0)
            ])
            ...
            # rotated_x = dir[0] * ti.cos(angle) + dir[1] * ti.sin(angle)
            # rotated_y = -dir[0] * ti.sin(angle) + dir[1] * ti.cos(angle)
            rotated_x = dir[0] * ti.cos(angle) - dir[1] * ti.sin(angle)
            rotated_y = dir[0] * ti.sin(angle) + dir[1] * ti.cos(angle)
            dir[0] = rotated_x
            dir[1] = rotated_y
            ...
            R = ti.Matrix.cols(
                [
                    ti.Vector([ti.cos(angle),  1. * ti.sin(angle), 0.,], dt=ti.float32), 
                    ti.Vector([-ti.sin(angle), ti.cos(angle), 0.], dt=ti.float32), 
            ...
            # 3 -> 2 matrix  # deformation'; # # 
            rotated_def = R @ self.deformation[fr, index_z, index_y, index_x] * flag
            #### select the deformation direction ## 
            # select_element_matrix = ti.Matrix.cols(
            #     [
            #         ti.Vector([0., 1.], dt=ti.float32), ti.Vector([1., 0.], dt=ti.float32), ti.Vector([0., 0.], dt=ti.float32)
            #     ]
            # ) ## ray march ##
            # # z up -> (y, x); y up -> (x, z)
            ### z is the main axis now # 
            select_element_matrix = ti.Matrix.cols(
                [
                    # ti.Vector([0., 1.], dt=ti.float32), ti.Vector([1., 0.], dt=ti.float32), ti.Vector([0., 0.], dt=ti.float32)
                    ti.Vector([0., 1.], dt=ti.float32), ti.Vector([0., 0.], dt=ti.float32), ti.Vector([1., 0.], dt=ti.float32)
                    # ti.Vector([1., 0.], dt=ti.float32), ti.Vector([0., 1.], dt=ti.float32), ti.Vector([0., 0.], dt=ti.float32)
                ]
            )
            selected_rotated_def = select_element_matrix @ rotated_def

            # contribution = self.density[index_z, index_y, index_x] * flag
            contribution = self.density_tot_frames[fr, index_z, index_y, index_x] * flag
            ## deformation ##
            
            # print(f"contribution: {contribution}, flag: {flag}, density: {self.density_tot_frames[fr, index_z, index_y, index_x]}")
            field[view_id, y, x] += contribution
            def_field[view_id, y, x] += selected_rotated_def * flag
    ...
    @ti.kernel
    def ray_march_single_loop_with_density_frames_with_deformation_yz(self, field: ti.template(), def_field: ti.template(), angle: ti.f32, view_id: ti.i32, fr: ti.i32): # run the render single loop #
        ## field; ## iamge; image deformation ##
        for pixel in range(self.res * self.res * self.marching_steps):
            # for k in range(self.marching_steps):
            x = (pixel % (self.res * self.res)) // self.res
            ...
            # rotated_z = -dir[0] * ti.sin(angle) + dir[2] * ti.cos(angle)
            # rotated x # dir 
            rotated_y = dir[1] * ti.cos(angle) - dir[2] * ti.sin(angle)
            rotated_z = dir[1] * ti.sin(angle) + dir[2] * ti.cos(angle)
            dir[1] = rotated_y
            dir[2] = rotated_z
            point = camera_origin + (k + 1) * self.dx * dir
            ...
            R = ti.Matrix.cols(
                [
                    ...
                    ti.Vector([0., ti.cos(angle),  1. * ti.sin(angle), ], dt=ti.float32), 
                    ti.Vector([0., -ti.sin(angle), ti.cos(angle)], dt=ti.float32)
                ]
            )
