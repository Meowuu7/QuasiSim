...
class Reconstructor:
    def __init__(self, args) -> None: 
        ...
        self.img_res = 512
        self.n_views = 1 #  100 # 14 # 100 #  14 #  7 # 14 ### 
        # self.buffered_grids = [
    ...
    def run_render(self, ):
        ...
        self.render.fill_density_deformation_fr_ti_with_fr_density_fr_single_loop(self.buffered_grids_deformation)
        for i_frame in range(self.n_timesteps):
            #### fill density fr ti ###
            t1 = time.time()
...
if __name__ == '__main__':
    ...
    def loss_and_grad(u): # compute loss and gradient through diff redmax
        ...
        if i_iter >= 1 and optim:
            ...
            sim.backward_info.df_dq = df_dq * 1 # * 1e3
        else:
            sim.backward_info.df_dq = df_dq
            
        for i_i in range(len(buffered_qs)):
            buffered_qs[i_i] = buffered_qs[i_i] - df_dqs_recon[i_i]
