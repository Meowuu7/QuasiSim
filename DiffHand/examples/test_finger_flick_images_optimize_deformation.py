...
class Reconstructor:
    def __init__(self, args) -> None: 
        ...
        self.n_views = 1 #  7 # 14 # 1 #  100 # 14 # 100 #  14 #  7 # 14 ### 
        # self.buffered_grids = [
    ...
    def get_loss_curr_to_target_rendered_images_with_frame_cd_th(self, cur_image: ti.template(), cur_image_def: ti.template(), target_image: ti.template()):
        cur_image_th = torch.from_numpy(cur_image.to_numpy()).float().cuda()
        cur_target_th = torch.from_numpy(target_image.to_numpy()).float().cuda()
        cur_image_grad = torch.zeros_like(cur_image_th)
        cur_image_def_th = torch.from_numpy(cur_image_def.to_numpy()).float().cuda()
        cur_image_def_grad = torch.zeros_like(cur_image_def_th)
        for i_v in range(cur_image_th.size(0)):
            cur_img_pts = []
            cur_img_pts_deformation = []
            # cur_img_pts_deformation = 
            cur_img_pts = torch.zeros((cur_image_th.size(1), cur_image_th.size(2), 2), dtype=torch.float32).cuda()
            cur_img_pts[:, :, 0] = torch.arange(cur_image_th.size(1)).float().cuda().unsqueeze(-1) / float(cur_image_th.size(1)) ## nn x 1 -> set the x axis
            cur_img_pts[:, :, 1] = torch.arange(cur_image_th.size(2)).float().cuda().unsqueeze(0) / float(cur_image_th.size(2)) ## nn x 1 -> set the x axis
            cur_img_pts = cur_img_pts[cur_image_th[i_v] > 0.]
            cur_img_pts_deformation = cur_image_def_th.clone()[i_v, cur_image_th[i_v] > 0.]
            ...
            cur_target_pts = torch.zeros((cur_image_th.size(1), cur_image_th.size(2), 2), dtype=torch.float32).cuda()
            cur_target_pts[:, :, 0] = torch.arange(cur_image_th.size(1)).float().cuda().unsqueeze(-1) / float(cur_image_th.size(1)) ## nn x 1 -> set the x axis
            cur_target_pts[:, :, 1] = torch.arange(cur_image_th.size(2)).float().cuda().unsqueeze(0) / float(cur_image_th.size(2)) ## nn x 1 -> set the x axis
            cur_target_pts = cur_target_pts[cur_target_th[i_v] > 0.]
            ...
            cur_img_pts_Xp = cur_img_pts * torch.tensor([cur_image_th.size(1), cur_image_th.size(2)], dtype=torch.float32).unsqueeze(0).cuda()
    ...
    def run_render(self, ):
        ...
        for i_frame in range(self.n_timesteps):
            ...
            for view in range(self.render.n_views): ### i_frame ###
                ...
                self.render.ray_march_single_loop_with_density_frames(self.buffered_images[i_frame], math.pi / (self.render.n_views * self.args.view_divide) * view - math.pi / 2.0, view, i_frame)
    ...
    def run_render_grad(self, ):
        ...
        for i_frame in reversed(range(self.n_timesteps)):
            for view in reversed(range(self.render.n_views)):
                ...
                self.render.ray_march_single_loop_with_density_frames.grad(self.buffered_images[i_frame], math.pi / (self.render.n_views * self.args.view_divide) * view - math.pi / 2.0, view, i_frame)
                # if not self.args.use_deformation:
    ...
    def rasterize_act_cur_step_cur_frame(self, i_link: ti.i32, i_fr: ti.i32): ## i_fr >= 1 here 
        for i_v in range(self.nn_samples):  ### nn_samples ###
            ...
            for offset in ti.static(ti.grouped(ti.ndrange(*self.neighbour))):
                ...
                #     self.buffered_grids_deformation[i_fr, base + offset] += weight * self.act_xs[i_link, i_fr, i_v]
                ## weight * def
    ...
    def backward_rasterize_passive_cur_step_cur_frame(self, i_fr: ti.i32):
        for i_v in range(self.nn_samples):      
            ...
            for offset in ti.static(ti.grouped(ti.ndrange(*self.neighbour))):
                ...
                #     # self.buffered_grids_deformation[i_fr, base + offset] += weight * self.passive_x_deformation[i_fr, i_v]
                #     self.buffered_grids_deformation[i_fr, base + offset] +=  self.passive_x_deformation[i_fr, i_v]
                # print(f"i_fr: {i_fr}, setting self.passive_x_deformation.grad[i_fr, i_v] to {weight * self.buffered_grids_deformation.grad[i_fr, base + offset]}")
...
if __name__ == '__main__':
    ...
    passive_only = False
    # passive_only = True ## passive only ####
    ...
    def loss_and_grad(u): # compute loss and gradient through diff redmax
        ...
        print(f"iter: {i_iter}, cost_to_goal: {cost_to_goal}, q[3]: {q[3]}, x_goal: {x_goal}")
