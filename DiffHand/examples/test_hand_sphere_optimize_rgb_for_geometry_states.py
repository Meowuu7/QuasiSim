...
import engine.reconstructor_dvgo_states as recon_states

from nerf import DirectVoxGO, get_rays_of_a_view, batch_indices_generator

# import engine.reconstructor_dvgo as recon_utils

##### load the reconstructor without passive object #####
### 

##### steps for using the reconstructor #####
# initialize state values #
# forward stepping #
# rasterize to the grids #
# optimize the density grid using images # -> and can draw the density grids out here #
##### iterate between # forward_dvgo_rendering and # backward_dvgo_rendering #####
...
if __name__ == '__main__':
    ...
    # ## load robots and load many other things # # assets # 
    xml_fn = os.path.join("/home/xueyi/diffsim/DiffHand/assets", f"{args.model}.xml")
    ...
    reconstructor = recon_states.Reconstructor(args=args, robot_name_to_robot=robot_name_to_robot)
    ...
    passive_robot_rigid_transformations = reconstructor.passive_robot_rigid_transformations
    ...
    tot_qs[:, -3:] = tot_qs[:, -3:]  * 0.
    tot_qs[:3, -3:] = passive_robot_rigid_transformations[:, -3:]
    
    
    reconstructor.set_states(tot_qs)
    reconstructor.forward_stepping()
    reconstructor.forward_tensor_to_ti()
    reconstructor.forward_rgb_density_rasterization()
    
    for i_iter in range(1000):
        reconstructor.forward_dvgo_rendering(rendering_timestep=rendering_timestep)
        reconstructor.backward_dvgo_rendering()
        f = reconstructor.tot_cd_loss
        print(f"i_iter: {i_iter}, f: {f}")
    ...
    def loss_and_grad_ref_states(u): ## loss and grad ref states
        ...
        # tot_qs[:, -6:] = passive_robot_rigid_transformations[:, :]
